#!/usr/bin/env bash
# run_evaluation_reanchor.sh — Re-anchor mode evaluation
set -e
...
N_SEGMENTS=6
SEED=99
REANCHOR_INTERVAL=25
OUTDIR="/tmp/eval_reanchor_$(date +%Y%m%d_%H%M)"
mkdir -p "$OUTDIR"

echo "Output: $OUTDIR"
echo "Mode: reanchor (interval=${REANCHOR_INTERVAL}s)"
...
run_clip() {
    ...
    python3 autopan_infer.py \
        ...
        --device mps \
        --mode reanchor \
        --reanchor-interval $REANCHOR_INTERVAL
