...
def component_smoke_check() -> Check:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            gate = detector_domain_gate(domain_eval, "insta360_style", 0.75)
            if gate.status != "pass":
                failures.append(f"domain detector gate failed: {gate}")
...
def detector_domain_gate(
    ...
    if not path.exists():
        return Check(
            ...
            label,
            "blocked_external",
            f"{rel(path)} missing; run train_ball_v5.py --eval-only --eval-domains for the final detector",
        )
    data = json.loads(path.read_text(encoding="utf-8"))
    fingerprint = data.get("weights_fingerprint")
    if weights_path is not None and isinstance(fingerprint, dict):
        if not weights_path.exists():
            return Check("verification", label, "blocked_external", f"{rel(path)} references missing {rel(weights_path)}")
        expected_sha = fingerprint.get("sha256")
        if expected_sha:
            actual_sha = sha256_file(weights_path)
            if actual_sha != expected_sha:
                return Check(
                    ...
                    label,
                    "blocked_external",
                    f"{rel(path)} does not match {rel(weights_path)} sha256={actual_sha[:12]} expected={expected_sha[:12]}; rerun domain eval",
                )
    rows = _domain_rows(data)
    row = next((item for item in rows if item.get("domain") == domain), None)
    if row is None:
        return Check("verification", label, "fail", f"{rel(path)} has no domain={domain!r} row")
    try:
        recall = float(row.get("recall", row.get("metrics/recall(B)", float("nan"))))
    except (TypeError, ValueError):
        recall = float("nan")
    if not math.isfinite(recall):
        return Check("verification", label, "fail", f"{rel(path)} domain={domain} has no finite recall")
    map50 = row.get("map50", row.get("mAP50", row.get("metrics/mAP50(B)")))
    map50_95 = row.get("map50_95", row.get("mAP50-95", row.get("metrics/mAP50-95(B)")))
    detail = f"{rel(path)} domain={domain} recall={recall:.3f}/{recall_threshold:.3f}"
...
def main() -> int:
    ...
    parser.add_argument("--detector-domain", default="insta360_style")
    parser.add_argument("--detector-domain-recall", type=float, default=0.75)
