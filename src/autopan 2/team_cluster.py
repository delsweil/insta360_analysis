...
class TeamClusterConfig:
    ...
    # Abstain on uncertain assignments to avoid polluting track votes.
    # Useful for referees/goalkeepers/edge cases.
    min_margin_for_label: float = 0.05
    max_best_dist_for_label: float = 6.0

    # Torso ROI (fractions of bbox)
    # Focus a bit higher on the body: more jersey, less shorts/socks.
    # This reduces confusion when the pitch/shorts dominate the lower torso.
    torso_y1: float = 0.08
    torso_y2: float = 0.48
    torso_xpad: float = 0.20

    # Pixel quality gates
    min_non_grass_frac: float = 0.10
    min_roi_px: int = 12*12
    # Make small/far players usable: normalize ROI size before feature extraction.
    roi_resize: int = 64  # 0 disables resizing
    ...
    grass_h_hi: int = 95
    grass_s_min: int = 40
    # Lower V threshold so shadowed grass is still masked as grass.
    grass_v_min: int = 15

    # Classification gating (helps avoid referee / low-quality ROIs polluting votes)
    min_margin_to_assign: float = 0.20
    max_best_dist_to_assign: float = 1.35
...
class TeamClusterer:
    ...
    def classify_box_with_margin(self, frame_bgr: np.ndarray, b):
        ...
        # Abstain if uncertain (prevents referees and noisy far ROIs from being
        # forced into a team bucket)
        if best > self.cfg.max_best_dist_for_label or margin < self.cfg.min_margin_for_label:
            return None, margin, best, second
    ...
    def classify_boxes_with_margins(self, frame_bgr: np.ndarray, boxes: List):
        ...
        for b in boxes:
            ...
            if best > self.cfg.max_best_dist_for_label or margin < self.cfg.min_margin_for_label:
                out.append((None, margin, best, second))
                continue
