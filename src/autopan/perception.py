...
class Detector:
    ...
    def detect_players_two_pass(
        ...
        mask_bin: Optional[np.ndarray] = None,
        # NOTE: too-high coverage thresholds can skip tiles near the pitch edge
        # (or under strong shadows) where the projected mask is imperfect.
        tile_pitch_min_coverage: float = 0.02,
        # limit pass2 to far band (top part of image)
        p2_ymax_frac: float = 0.55,
        # If pass1 finds very few players, widen the pass2 band a bit to avoid
        # missing mid-field players that fall just below p2_ymax_frac.
        p2_ymax_frac_low_players: Optional[float] = 0.70,
        # optional throttle
        ...
        run_p2 = False
        reason_low_players = False
        if len(p1) < trigger_min_players:
            run_p2 = True
            reason_low_players = True
        else:
        ...
        use_frac = float(p2_ymax_frac)
        if reason_low_players and p2_ymax_frac_low_players is not None:
            use_frac = float(p2_ymax_frac_low_players)
        p2_ymax = int(max(64, min(h, round(h * use_frac))))
