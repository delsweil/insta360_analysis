...
NAME="${NAME:-ball_v5_yolo11s_1536_highres}"
EPOCHS="${EPOCHS:-180}"
BATCH="${BATCH:-8}"
WORKERS="${WORKERS:-8}"
DEVICE="${DEVICE:-0}"
...
echo "[highres start] $(date -u +%Y-%m-%dT%H:%M:%SZ)"
...
if [[ -n "$WAIT_PID" ]]; then
  while pid_is_running "$WAIT_PID"; do
    echo "[highres wait] $(date -u +%Y-%m-%dT%H:%M:%SZ) pid=$WAIT_PID still running"
    sleep "${WAIT_INTERVAL_SECONDS:-300}"
  done
  echo "[highres wait] pid=$WAIT_PID exited"
fi
...
PY
  ...
  echo "[highres status] previous finalize status=$STATUS"
  if [[ "$STATUS" == "passed" ]]; then
    echo "[highres skip] existing run passed target gate"
    exit 0
  fi
fi

echo "[highres train] model=$MODEL imgsz=$IMGSZ name=$NAME epochs=$EPOCHS batch=$BATCH"
nvidia-smi || true
...
echo "[highres complete] $(date -u +%Y-%m-%dT%H:%M:%SZ)"
